 ...
 */
@Internal
public class StrengthenKafkaFetcher<T> extends AbstractFetcher<T, TopicPartition> {

	private static final Logger LOG = LoggerFactory.getLogger(StrengthenKafkaFetcher.class);
	...
	/**
	 * The thread that runs the actual KafkaConsumer and hand the record batches to
	 * this fetcher.
	 */
	private final StrengthenKafkaConsumerThread consumerThread;
	...
	public StrengthenKafkaFetcher(SourceContext<T> sourceContext,
			Map<KafkaTopicPartition, Long> assignedPartitionsWithInitialOffsets,
			SerializedValue<AssignerWithPeriodicWatermarks<T>> watermarksPeriodic,
			SerializedValue<AssignerWithPunctuatedWatermarks<T>> watermarksPunctuated,
			ProcessingTimeService processingTimeProvider, long autoWatermarkInterval, ClassLoader userCodeClassLoader,
			String taskNameWithSubtasks, KafkaDeserializationSchema<T> deserializer, Properties kafkaProperties,
			long pollTimeout, MetricGroup subtaskMetricGroup, MetricGroup consumerMetricGroup, boolean useMetrics,
			FlinkConnectorRateLimiter rateLimiter, boolean isBounded) throws Exception {
		super(sourceContext, assignedPartitionsWithInitialOffsets, watermarksPeriodic, watermarksPunctuated,
				processingTimeProvider, autoWatermarkInterval, userCodeClassLoader, consumerMetricGroup, useMetrics);
		this.deserializer = deserializer;
		this.handover = new Handover();
		this.consumerThread = new StrengthenKafkaConsumerThread(LOG, handover, kafkaProperties,
				unassignedPartitionsQueue, createCallBridge(), getFetcherName() + " for " + taskNameWithSubtasks,
				pollTimeout, useMetrics, consumerMetricGroup, subtaskMetricGroup, rateLimiter,isBounded);
	}
		...
		try {
			...
			while (running) {
				...
				for (KafkaTopicPartitionState<TopicPartition> partition : subscribedPartitionStates()) {
					...
					List<ConsumerRecord<byte[], byte[]>> partitionRecords = records
							.records(partition.getKafkaPartitionHandle());
				...
				if (this.consumerThread.reachedEnd()) {
					running = false;
				}
			}
		} finally {
	...
	protected StrengthenKafkaConsumerCallBridge createCallBridge() {
		return new StrengthenKafkaConsumerCallBridge();
	}

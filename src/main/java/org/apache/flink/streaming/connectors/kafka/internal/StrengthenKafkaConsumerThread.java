...
import org.apache.flink.kafka011.shaded.org.apache.kafka.clients.consumer.ConsumerRecord;
import org.apache.flink.kafka011.shaded.org.apache.kafka.clients.consumer.ConsumerRecords;
import org.apache.flink.kafka011.shaded.org.apache.kafka.clients.consumer.KafkaConsumer;
import org.apache.flink.kafka011.shaded.org.apache.kafka.clients.consumer.OffsetAndMetadata;
import org.apache.flink.kafka011.shaded.org.apache.kafka.clients.consumer.OffsetCommitCallback;
import org.apache.flink.kafka011.shaded.org.apache.kafka.common.Metric;
...
import org.apache.flink.kafka011.shaded.org.apache.kafka.common.errors.WakeupException;
...
/**
 * The thread the runs the {@link KafkaConsumer}, connecting to the brokers and
 ...
 * it down. The Kafka consumer code was found to not always handle interrupts
 ...
 * the KafkaConsumer. Because Kafka is not maintaining binary compatibility, we
 * use a "call bridge" as an indirection to the KafkaConsumer calls that change
 * signature.
 */
@Internal
public class StrengthenKafkaConsumerThread extends Thread {
	...
	/**
	 * The queue of unassigned partitions that we need to assign to the Kafka
	 * consumer.
	 */
	private final ClosableBlockingQueue<KafkaTopicPartitionState<TopicPartition>> unassignedPartitionsQueue;

	/**
	 * The indirections on KafkaConsumer methods, for cases where KafkaConsumer
	 * compatibility is broken.
	 */
	private final StrengthenKafkaConsumerCallBridge consumerCallBridge;
	...
	/**
	 * Flag to indicate whether an external operation
	 * ({@link #setOffsetsToCommit(Map, KafkaCommitCallback)} or
	 * {@link #shutdown()}) had attempted to wakeup the consumer while it was
	...
	public StrengthenKafkaConsumerThread(Logger log, Handover handover, Properties kafkaProperties,
			ClosableBlockingQueue<KafkaTopicPartitionState<TopicPartition>> unassignedPartitionsQueue,
			StrengthenKafkaConsumerCallBridge consumerCallBridge, String threadName, long pollTimeout,
			boolean useMetrics, MetricGroup consumerMetricGroup, MetricGroup subtaskMetricGroup,
			FlinkConnectorRateLimiter rateLimiter, boolean isBounded) {
	...
	public void run() {
		...
		// This method initializes the KafkaConsumer and guarantees it is torn down
		// properly.
			...
			while (running) {
				...
				try {
					if (hasAssignedPartitions) {
						...
						newPartitions = unassignedPartitionsQueue.getBatchBlocking(pollTimeout);
					}
	...
	void reassignPartitions(List<KafkaTopicPartitionState<TopicPartition>> newPartitions) throws Exception {
		...
		// since the reassignment may introduce several Kafka blocking calls that cannot
		// be interrupted,
		...
		try {
			...
			synchronized (consumerReassignmentLock) {
				...
				if (reassignmentStarted) {
					consumerCallBridge.assignPartitions(this.consumer,
	...
	/**
	 * Get records from Kafka. If the rate-limiting feature is turned on, this
	 * method is called at a rate specified by the {@link #rateLimiter}.
	 * 
	...
	protected ConsumerRecords<byte[], byte[]> getRecordsFromKafka() {
		...
		if(!reachedEnd) {
			...
			if ((null == records || records.isEmpty()) && isBounded) {
				Set<TopicPartition> topicPartitions = consumer.assignment();
				Map<TopicPartition, Long> endOffsets = consumer.endOffsets(topicPartitions);
				int allReachedEndCount=0;
				for (TopicPartition topicPartition : topicPartitions) {
					OffsetAndMetadata offsetAndMetadata = consumer.committed(topicPartition);
					long endoffset = endOffsets.get(topicPartition);
					if (endoffset<=1||(null != offsetAndMetadata&&offsetAndMetadata.offset() >= endoffset)) {
						allReachedEndCount++;
					}
				}
				reachedEnd=topicPartitions.size()==allReachedEndCount;
			}
			if (rateLimiter != null) {
